...
async function createEmbeds(commandData){
    ...
    for (const [key, value] of Object.entries(commandData)){
        ...
        for (const index of value){
            finalString = finalString.concat(`</${index[0]}:${index[1]}> - ${index[2]}\n`)
        }
        embeds.push(
            new EmbedBuilder()
    ...
    for (let i = 0; i < embeds.length; i++){
        let prev = ""
        let next = ""
        if (embeds[i-1] !== undefined) prev = '⏪' + embeds[i-1].data.title;

        if (embeds[i+1] !== undefined) next = embeds[i+1].data.title + '⏩';
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        var commandIds = Object

        all_commands = await interaction.guild.commands.fetch();

        for (const [key, value] of all_commands){
            commandIds[value.name] = key
        }
        ...
        for (const subFolder of commandFolders){
            ...
            for (const file of commandFiles){
                ...
                if (cmd.enabled){
                    commandData[cmd.category].push([cmd.data.name, commandIds[cmd.data.name], cmd.data.description])
                }
            }
        }
